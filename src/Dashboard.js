...
export default function Dashboard({ data }) {
  ...
  // --- 🇹🇭 ฐานข้อมูลคำแปลเมนูไทยเพื่อให้คำนวณแม่นยำขึ้น ---
  const THAI_MENU_MAP = {
  ...
  const getRealNutrition = async (foodName) => {
    // ตรวจสอบว่ามีชื่อเมนูที่เฉพาะเจาะจงในแผนที่ภาษาไทยไหม
    ...
    try {
      ...
      return {
        name: searchQuery,
        cal: res.data.calories.value,
        p: res.data.protein.value,
        c: res.data.carbs.value,
        f: res.data.fat.value,
      };
    } catch (err) {
      return null;
    }
      ...
      try {
        ...
        for (let item of targetItems) {
          ...
          if (nut) {
            ...
            itemsDetected.push(nut.name); // เก็บชื่อเมนูที่แปลแล้วไปแสดงผล
          }
        }

        if (itemsDetected.length > 0) {
          alert(
            `ตรวจพบ: ${itemsDetected.join(", ")}\nพลังงานรวม: ${Math.round(
              mealSum.cal
            )} kcal`
          );
          setEaten((prev) => ({
  ...
  return (
    <div style={containerStyle}>
        ...
        <div style={ringWrapper}>
          <svg width="180" height="180" viewBox="0 0 160 160">
            ...
            <circle
              ...
              strokeDasharray="440"
              strokeDashoffset={
                440 - (440 * (progress > 100 ? 100 : progress)) / 100
              }
              strokeLinecap="round"
          ...
          <div style={insideText}>
            <span
              style={{ fontSize: "32px", fontWeight: "800", color: "#333" }}
            >
      ...
      >
        ...
        {isScanning ? "กำลังวิเคราะห์เมนูไทย..." : "คำนวนCalorie"}
        {!isScanning && (
          <input
            type="file"
            accept="image/*"
...
const fabStyle = {
  ...
  justifyContent: "center",
  alignItems: "center",
  boxShadow: "0 10px 20px rgba(255, 122, 48, 0.3)",
};
